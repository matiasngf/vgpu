...
describe.skipIf(process.env.VGPU_DOCKER_TEST !== "1")("storage textures on Dawn", () => {
  test("compute writes a 2D storage texture that reads back", async () => {
    ...
    try {
      const out = gpu.texture({ size: [8, 8], format: "rgba8unorm", label: "fill-2d" });
      gpu.compute(FILL_2D, { label: "fill-2d", set: { out } }).dispatch(1, 1);
  ...
  test("compute writes a 3D storage texture sampled by a fragment shader", async () => {
    const gpu = await init();
    try {
      const lut = gpu.texture({ size: [4, 4, 4], format: "rgba16float", dimension: "3d", label: "lut" });
      gpu.compute(FILL_3D, { label: "fill-3d", set: { lut } }).dispatch(1, 1, 1);
      ...
      gpu.frame((frame) => frame.pass({ target, clear: [0, 0, 0, 1] }, (pass) => pass.draw(view)));
