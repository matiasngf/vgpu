...
describe.skipIf(process.env.VGPU_DOCKER_TEST !== "1")("sampleable depth on Dawn", () => {
  test("depth written by a draw is read back by a later effect", async () => {
    ...
    try {
      const scene = gpu.target({ size: [8, 8], format: "rgba8unorm", depth: true, label: "scene" });
      const output = gpu.target({ size: [8, 8], format: "rgba8unorm", label: "output" });
      const geometry = gpu.draw({ shader: WRITE_DEPTH, vertices: 3, targets: [scene] });
      const fog = gpu.effect(READ_DEPTH, { label: "fog", set: { sceneDepth: scene, depthCompare: gpu.sampler({ compare: "less" }) } });
      gpu.frame((frame) => {
        frame.pass({ target: scene, clear: [0, 0, 0, 1] }, (pass) => pass.draw(geometry));
        frame.pass({ target: output, clear: [0, 0, 0, 1] }, (pass) => pass.draw(fog));
      });
