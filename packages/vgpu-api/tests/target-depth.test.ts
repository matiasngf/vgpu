...
describe("sampleable target depth", () => {
  test("depth attachments carry texture_binding usage", async () => {
    gpu = await init();
    const target = gpu.target({ size: [32, 32], depth: true });
    expect(target.depth?.format).toBe("depth24plus");
    expect([...(target.depth?.usage ?? [])]).toEqual(expect.arrayContaining(["render_attachment", "texture_binding"]));
  });
  ...
  test("target.depth binds to texture_depth_2d with a depth sample type", async () => {
    ...
    const scene = gpu.target({ size: [32, 32], depth: true });
    const output = gpu.target({ size: [32, 32] });
    const fog = gpu.effect(DEPTH_READ, { label: "fog", set: { sceneDepth: scene.depth } });
    expect(() => gpu!.frame((frame) => frame.pass({ target: output }, (pass) => pass.draw(fog)))).not.toThrow();
  ...
  test("a sampler paired with a depth texture gets a non-filtering layout", async () => {
    ...
    const layoutSpy = vi.spyOn(device, "createBindGroupLayout");
    const scene = gpu.target({ size: [32, 32], depth: true });
    gpu.effect(DEPTH_SAMPLE, { label: "fog-sample", set: { sceneDepth: scene, depthSampler: gpu.sampler({ minFilter: "nearest", magFilter: "nearest" }) } });
  ...
  test("a Target bound to a depth binding resolves to its depth texture", async () => {
    ...
    gpu.effect(DEPTH_READ, { label: "fog", set: { sceneDepth: scene } });
  ...
  test("a Target without depth is rejected with a fix-it", async () => {
    gpu = await init();
    const scene = gpu.target({ size: [32, 32] });
    const fog = gpu.effect(DEPTH_READ, { label: "fog" });
  ...
  test("stencil formats are bound through a depth-only view", async () => {
    gpu = await init();
    const scene = gpu.target({ size: [32, 32], depth: "depth24plus-stencil8" });
    const spy = vi.spyOn(scene.depth!.gpu, "createView");
    gpu.effect(DEPTH_READ, { label: "fog", set: { sceneDepth: scene.depth } });
  ...
  test("resizing a target rebinds the recreated depth texture", async () => {
    ...
    const fog = gpu.effect(DEPTH_READ, { label: "fog", set: { sceneDepth: scene } });
    scene.resize([64, 64]);
    expect(() => gpu!.frame((frame) => frame.pass({ target: output }, (pass) => pass.draw(fog)))).not.toThrow();
    await gpu.settled();
  });
