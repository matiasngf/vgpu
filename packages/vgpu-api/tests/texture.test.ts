...
describe("gpu.texture()", () => {
  test("creates a 2D texture with sampled + storage + copy usage by default", async () => {
    gpu = await init();
    const texture = gpu.texture({ size: [16, 8], format: "rgba8unorm", label: "noise" });
    expect(texture.size).toEqual([16, 8]);
    expect(texture.format).toBe("rgba8unorm");
    expect(texture.dimension).toBe("2d");
    expect([...texture.usage].sort()).toEqual(["copy_dst", "copy_src", "storage_binding", "texture_binding"]);
    expect(texture.label).toBe("noise");
    texture.destroy();
  });
  ...
  test("creates a 3D texture when dimension is 3d", async () => {
    ...
    const lut = gpu.texture({ size: [32, 32, 32], format: "rgba16float", dimension: "3d" });
  ...
  test("explicit usage replaces the default", async () => {
    gpu = await init();
    const atlas = gpu.texture({ size: [4, 4, 8], format: "bgra8unorm", usage: ["texture_binding", "copy_dst"] });
  ...
  test("throws VGPU-TEXTURE-SIZE-REQUIRED without a valid size", async () => {
    gpu = await init();
    const g = gpu;
    expect(codeOf(() => g.texture({ format: "rgba8unorm" } as never))).toBe("VGPU-TEXTURE-SIZE-REQUIRED");
    expect(codeOf(() => g.texture({ size: [0, 4], format: "rgba8unorm" }))).toBe("VGPU-TEXTURE-SIZE-REQUIRED");
    expect(codeOf(() => g.texture({ size: [4, 4, 0], format: "rgba8unorm" }))).toBe("VGPU-TEXTURE-SIZE-REQUIRED");
  });

  test("throws VGPU-TEXTURE-STORAGE-FORMAT for a non-storage format with storage_binding usage", async () => {
    gpu = await init();
    const g = gpu;
    expect(codeOf(() => g.texture({ size: [4, 4], format: "bgra8unorm" }))).toBe("VGPU-TEXTURE-STORAGE-FORMAT");
    expect(messageOf(() => g.texture({ size: [4, 4], format: "rgb10a2unorm" }))).toMatch(/storage_binding/);
    expect(() => g.texture({ size: [4, 4], format: "bgra8unorm", usage: ["texture_binding"] })).not.toThrow();
  });
});
...
describe("storage texture bindings", () => {
  test("compute accepts a storage texture and reflects the layout", async () => {
    ...
    const out = gpu.texture({ size: [16, 16], format: "rgba8unorm" });
    const fill = gpu.compute(STORAGE_2D, { label: "fill" });
    fill.set({ out });
  ...
  test("a 3D storage texture binds to texture_storage_3d and to texture_3d", async () => {
    gpu = await init();
    const lut = gpu.texture({ size: [8, 8, 8], format: "rgba16float", dimension: "3d" });
    const fill = gpu.compute(STORAGE_3D, { label: "fill-3d", set: { lut } });
    expect(() => fill.dispatch(2, 2, 2)).not.toThrow();
    const target = gpu.target({ size: [8, 8], format: "rgba8unorm" });
    const view = gpu.effect(SAMPLE_3D, { label: "view-3d", set: { lut, linear: gpu.sampler() } });
    expect(() => gpu!.frame((frame) => frame.pass({ target }, (pass) => pass.draw(view)))).not.toThrow();
  });

  test("storage view binds mip level 0 only", async () => {
    gpu = await init();
    const mipped = gpu.texture({ size: [16, 16], format: "rgba8unorm", mipLevelCount: 3 });
    const spy = vi.spyOn(mipped.gpu, "createView");
    gpu.compute(STORAGE_2D, { label: "fill-mips", set: { out: mipped } });
    const desc = spy.mock.calls.at(-1)?.[0];
  ...
  test("rejects a Target for a storage texture binding", async () => {
    ...
    const fill = gpu.compute(STORAGE_2D, { label: "fill" });
    expect(codeOf(() => fill.set({ out: target }))).toBe("VGPU-R1-BINDING-INCOMPATIBLE-RESOURCE");
    expect(messageOf(() => fill.set({ out: target }))).toMatch(/gpu\.texture\(/);
  });

  test("rejects a texture without storage_binding usage", async () => {
    gpu = await init();
    const sampledOnly = gpu.texture({ size: [16, 16], format: "rgba8unorm", usage: ["texture_binding"] });
    const fill = gpu.compute(STORAGE_2D, { label: "fill" });
  ...
  test("rejects a format that differs from the WGSL declaration", async () => {
    gpu = await init();
    const wrongFormat = gpu.texture({ size: [16, 16], format: "rgba16float" });
    const fill = gpu.compute(STORAGE_2D, { label: "fill" });
  ...
  test("rejects a dimension that differs from the WGSL declaration", async () => {
    gpu = await init();
    const flat = gpu.texture({ size: [8, 8], format: "rgba16float" });
    const fill = gpu.compute(STORAGE_3D, { label: "fill-3d" });
  ...
  test("destroying a bound storage texture evicts its bind group identity", async () => {
    gpu = await init();
    const out = gpu.texture({ size: [16, 16], format: "rgba8unorm" });
    const fill = gpu.compute(STORAGE_2D, { label: "fill", set: { out } });
    fill.dispatch(1);
    out.destroy();
    const replacement = gpu.texture({ size: [16, 16], format: "rgba8unorm" });
