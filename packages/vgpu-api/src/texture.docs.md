...
Creates a standalone sampleable/storage texture from the main API (`vgpu`). Use it when a shader needs a texture that is not a render target: compute-written storage textures, 3D lookup tables, texture arrays, or mipmapped inputs. Render outputs still use `gpu.target()`.
...
const lut = gpu.texture({ size: [32, 32, 32], format: "rgba16float", dimension: "3d", label: "lut" });
...
`, { label: "view-lut", set: { lut, linear: gpu.sampler() } });
```
...
const atlas = gpu.texture({ size: [256, 256, 8], format: "bgra8unorm", usage: ["texture_binding", "copy_dst"] });
atlas.destroy();
```
...
- A `Target` cannot satisfy a storage texture binding; render attachments are not storage textures. Write into `gpu.texture()` from compute and sample it from the render pass instead.
...
- **See also:** `Gpu.target`, `Gpu.compute`, `Compute.set`, `Texture` from `vgpu/core`.
