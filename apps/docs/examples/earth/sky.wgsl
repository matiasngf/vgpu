...
//
// The three.js experiment rendered a second scene into an FBO — an equirectangular
// star photo converted to a cube map, with a small emissive sphere for the sun —
// and used it as `scene.background`. Both collapse into one fullscreen effect here:
// stars come from an integer hash over cube-face cells, and the sun is an angular
// disk around the light direction.
//
...
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  // --- Stars ---------------------------------------------------------------
  // Three layers: sparse bright anchors, the main field, and a dense dim wash
