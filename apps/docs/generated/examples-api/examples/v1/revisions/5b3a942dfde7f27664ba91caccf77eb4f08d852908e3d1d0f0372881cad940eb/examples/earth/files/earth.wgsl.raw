// The planet surface. A direct translation of the experiment's GLSL
// `earthFragmentShader`: day albedo gated by a remapped Lambert term, night
...
//   * `varying` becomes an explicit `VertexOut`, and `cameraPosition` — a free
//     uniform in three.js — is passed in on the uniform block.
//   * `dFdx`/`dFdy` become `dpdx`/`dpdy`. All texture sampling stays in uniform
...
fn fs_main(input: VertexOut) -> @location(0) vec4f {
  ...
  // The original clamps the planet to 0.7 so it always stays under the bloom
