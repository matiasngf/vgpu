...
- Readback only supports 8-bit formats, so examples preview HDR targets through a tonemapping pass first (`spaceship-thrusters/debug-preview.wgsl` is a reusable template). `spaceship-thrusters` reports `noise-atlas`, `detail`, `shadow-map`, `scene-color`, `scene-depth`, `scene-normal`, `plume-grid`, `fire-hdr`, and `bloom`, plus `ao`, `scene-lit` and `composite` in the social pipeline.
