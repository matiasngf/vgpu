...
- **Atmosphere stills:** `node scripts/render-atmosphere.mjs --out <dir> [--preset all|golden-hour|noon|twilight|high-altitude|stratosphere] [--size 960x540] [--debug transmittance|multiscatter|sky-view|weather|terrain] [--bench N] [--sun 4 --altitude 10 --ev 5 --haze 2 --coverage 0.5 --detail 1 --type 0 --seed 3 --time 0 --yaw 0 --pitch 6 --tonemap agx]` renders the atmosphere example headless with any preset or override and prints per-band colour averages, which is the fastest way to check a change to the sky model visually. `--bench N` reports steady-state ms per frame with and without clouds on one graph, and `--accumulate N` renders the live-loop path (jittered history accumulation) for N frames instead of a deterministic still. `--bench N` renders N steady-state frames on one graph and prints ms/frame with and without clouds.
